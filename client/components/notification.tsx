...
export default function Notification() {
      ...
      socket.on("new_notification", (data: INotification) => {
        ...
        if (toastData) {
          toast({
            ...
            description: a(toastData.description, {
  ...
  const trailingActions = (id: number) => (
    <TrailingActions>
      <SwipeAction destructive={true} onClick={() => removeNotification(id)}>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger>
        <Button
          variant="ghost"
          className="w-[45px] h-[45px] text-input relative"
        >
        ...
        <Card className="flex flex-col w-[480px] h-[580px] border-none px-6 py-4 gap-4">
          <CardHeader className="p-0 flex gap-0">
            <CardTitle className="flex flex-col">
              ...
              <p className="p-0 text-base font-normal text-muted-foreground">
                {t("YouHaveUnreadMessages", { count: unreadCount })}
              </p>
            </CardTitle>
          </CardHeader>
                      ...
                      >
                        <span
                          className={`h-3 w-3 rounded-full ${
                            notification.read ? "bg-input" : "bg-primary"
                          }`}
                        />
