...
import { fetchData, getInitials, getItemTypeVariant, sortData } from '@/lib/utils';
...
export default function Page() {
    ...
    const buildQueryString = (filter: FilterComment | null, searchTerm: string) => {
                        ...
                        <div>
                            ...
                            >
                                <SelectTrigger className="">
                                    <SelectValue
                                        placeholder={filter?.commentStatus === 'All' ? t('All') : filter?.commentStatus}
                                    />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectGroup>
                ...
                <TableBody>
                    {allComments.map((comment, index) => (
                        <TableRow key={index}>
                            ...
                            <TableCell className="font-medium">{formatTime(comment.timestamp)}</TableCell>
                            <TableCell className="font-medium">
                                <BadgeCustom
                                    variant={
                                        comment.status === false ? "blue" : "green"
                                    }
                                    iconName={
                                            ...
                                            <AlertDialog>
                                                ...
                                                <AlertDialogContent className='px-6 py-4 gap-4'>
                                                    <div className='flex flex-col gap-2'>
                                                        <p className='text-lg font-semibold text-muted-foreground'>{formatTime(comment.timestamp)}</p>
                                                        <p className='text-2xl font-bold text-card-foreground'>{comment.patrolResult.itemZone.item.checklist.title}</p>
                                                    </div>
                                                    <div className='flex flex-col gap-2'>
                                                        <p className='text-xl text-muted-foreground'>{comment.patrolResult.itemZone.item.name}</p>
                                                        <div className='flex items-center gap-1 text-base text-muted-foreground'>
                                                            <span className="material-symbols-outlined">
                                                                location_on
                                                            </span>
                                                            <p>{z(comment.patrolResult.itemZone.zone.name)}</p>
                                                        </div>
                                                    </div>
                                                    <BadgeCustom
                                                        variant={getItemTypeVariant(comment.patrolResult.itemZone.item.type as itemType).variant}
                                                        shape="square"
                                                        showIcon={true}
                                                        iconName={getItemTypeVariant(comment.patrolResult.itemZone.item.type as itemType).iconName}
                                                    >
                                                ...
                                                <AlertDialogContent>
                                                    <AlertDialogHeader>
                                                        ...
                                                        <AlertDialogDescription className='text-base text-card-foreground'>
                                                            Are you sure you want to acknowledge this comment?
